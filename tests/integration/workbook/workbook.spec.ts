...
/**
 * Check array has all expected members (order independent)
 */
function expectArrayMembers(actual: any[], expected: any[]): void {
  expect(actual.sort()).toEqual(expected.sort());
}

const TEST_XLSX_FILE_NAME = './tests/out/wb.test.xlsx';
const TEST_CSV_FILE_NAME = './tests/out/wb.test.csv';
...
describe('Workbook', () => {
  describe('Serialise', () => {
    ...
    it('creator, lastModifiedBy, etc', () => {
      ...
      return wb.xlsx
        ...
        .then(wb2 => {
          ...
          expectDateEqual(wb2.created, wb.created);
          expectDateEqual(wb2.modified, wb.modified);
        });
    });
    it('printTitlesRow', () => {
      const wb = new ExcelTS.Workbook();
    ...
    it('defined names', () => {
      const wb1 = new ExcelTS.Workbook();
      ...
      return wb1.xlsx
        ...
        .then(wb2 => {
          ...
          // two names
          expectArrayMembers(ws2a.getCell('G1').names, [
            'thing1',
            'thing2',
          ]);
      ...
      it('serialises and deserialises properly', () => {
        ...
        return wb.xlsx
          ...
          .then(wb2 => {
            ...
            expect(ws2.getCell('B2').type).toBe(ExcelTS.ValueType.String);
            expect(ws2.getCell('B3').type).toBe(ExcelTS.ValueType.Merge);
            expect(ws2.getCell('C2').type).toBe(ExcelTS.ValueType.Merge);
            expect(ws2.getCell('C3').type).toBe(ExcelTS.ValueType.Merge);
          });
      });
